...
, WithSnippets(..)
, getCompletions
,getKeywordCompletions,getOptionCompletions) where
...
getCompletions ideOpts CC { allModNamesAsNS, unqualCompls, qualCompls, importableModules}
  ...
  let VFS.PosPrefixInfo { fullLine, prefixModule, prefixText } = prefixInfo
      ...
      filtImportCompls = filtListWith fullPrefix enteredQual (mkImportCompl enteredQual) importableModules
...
filtListWith :: T.Text -> T.Text -> (T.Text -> a) -> [T.Text] -> [a]
filtListWith fullPrefix enteredQual f list =
  [ f label
...
getKeywordCompletions ::
  ...
  VFS.PosPrefixInfo ->
  [CompletionItem]
getKeywordCompletions ideOpts prefixInfo = do
  let VFS.PosPrefixInfo {fullLine, prefixModule, prefixText} = prefixInfo
      enteredQual = if T.null prefixModule then "" else prefixModule <> "."
      fullPrefix = enteredQual <> prefixText
      filtKeywordCompls
        | T.null prefixModule = filtListWith fullPrefix enteredQual mkExtCompl (optKeywords ideOpts)
        | otherwise = []
...
getOptionCompletions ::
  ...
  WithSnippets ->
  [CompletionItem]
getOptionCompletions prefixInfo caps withSnippets = do
  let VFS.PosPrefixInfo {fullLine, prefixModule, prefixText} = prefixInfo
      enteredQual = if T.null prefixModule then "" else prefixModule <> "."
      fullPrefix = enteredQual <> prefixText

      filtOptsCompls = filtListWith fullPrefix enteredQual mkExtCompl
